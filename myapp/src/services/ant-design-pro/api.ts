...
export async function currentUser(options?: { [key: string]: any }) {
  ...
  console.log('调用获取用户信息 API, token:', token);
  const response = await request<API.CurrentUser>('/api/auth/profile', {
    method: 'GET',
    headers: {
  ...
  console.log('获取用户信息 API 响应:', response);
  return response;
}
